...
class EmailMgr:
    def send_mail(self):
        ...
        MESSAGE['subject'] = "Comcast OSS || RELEASE PONR Report || Proactive Handling to avoid Rebill " + dt.datetime.strftime(dt.datetime.now(), "%Y/%m/%d")
    ...
    def get_mail_content(self):
        ...
        <body>
        <p>Hi Team,</p>
        <p>Please find below the list of orders where the RELEASE PONR flag was not triggered by the system or the "Set Product PONR" activity status not marked completed, even though the mandatory wait period associated with the Registered Timed Action has been completed</p>
        <p>Kindly review these orders and take the necessary corrective actions. This issue may be impacting the business, as services have already been ceased at the customer sites, but billing continues to be active.</p>
        {html_table}
