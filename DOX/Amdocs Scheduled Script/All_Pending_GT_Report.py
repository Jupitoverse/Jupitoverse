...
def fetch_pending_tasks_batch(cursor, part_id_batch):
    ...
    try:
        ...
        logging.info(f"Prepared {len(spec_list)} spec_ver_ids (filtered out _WA entries)")
    except Exception as e:
        logging.error(f"Error preparing spec_ver_id list: {e}")
        raise
    ...
    try:
        # Check if cursor is still open
        if cursor.closed:
            logging.error(f"[ERROR] Cursor is closed before executing batch {part_id_batch}")
            return []
        ...
        logging.info(f"[OK] BATCH COMPLETE: Fetched {record_count} records for part_id {part_id_batch}")
        ...
        logging.error(f"[ERROR] Database interface error in batch {part_id_batch}: {str(e)}")
        logging.error(f"Cursor closed: {cursor.closed}")
        logging.error(f"This is likely due to a previous error. Stopping batch processing.")
        return []
    except Exception as e:
        logging.error(f"[ERROR] ERROR in batch {part_id_batch}")
...
def fetch_workqueue(cursor, activity_id):
    ...
    try:
        ...
        if result:
            workqueue = result[0]
            logging.info(f"    [OK] Found WorkQueue: {workqueue}")
            return workqueue
        else:
            logging.warning(f"    [X] No WorkQueue found in database")
        ...
        logging.error(f"    [X] Database error for activity_id={activity_id}: {e}")
...
def create_summary_table(df):
    ...
    logging.info(f"[OK] Summary table created with {len(summary_df)} rows")
...
def pull_all_pending_tasks():
    ...
    try:
        ...
        for i in range(0, len(all_part_ids), batch_size):
            ...
            # Check if cursor is still valid before processing
            if cursor.closed:
                logging.error("[ERROR] Cursor is closed. Cannot continue processing batches.")
                logging.error("This likely indicates a database error in a previous batch.")
                break
            
            results = fetch_pending_tasks_batch(cursor, part_id_batch)
            
            # Check if we got an error (cursor closed)
            if results is None or (len(results) == 0 and cursor.closed):
                logging.error("[ERROR] Batch returned no results and cursor is closed. Stopping.")
                break
        ...
        logging.info("-" * 80)
        logging.info(f"[OK] DATA FETCH COMPLETE")
        logging.info(f"[OK] TOTAL RECORDS FETCHED: {len(all_results)}")
        logging.info("=" * 80)
        
        # Convert to DataFrame
        if all_results:
            df = pd.DataFrame(all_results, columns=column_names)
            logging.info(f"[OK] DataFrame created successfully with {len(df)} rows and {len(df.columns)} columns")
            ...
            logging.info(f"[OK] Age column added (range: {df['Age'].min()} to {df['Age'].max()} days)")
...
def generate_charts_html(summary_df):
    ...
    <div style="background-color: white; padding: 20px; margin: 20px 0; border-radius: 5px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
        <h3 style="color: #004080; margin-top: 0;">[Chart] Task Distribution Overview</h3>
...
def generate_html_report(df, summary_df=None):
    ...
    if summary_df is not None and len(summary_df) > 0:
        summary_html = f"""
        <h2 style="color: #004080; margin-top: 30px;">[Summary] Summary by Task Type</h2>
...
class EmailMgr:
    ...
    def send_mail(self):
        ...
        logging.info(f"[OK] HTML body attached (size: {len(self.html_content)} bytes)")
        ...
        if self.excel_file and os.path.exists(self.excel_file):
            try:
                ...
                logging.info(f"[OK] Excel file attached: {filename}")
            except Exception as e:
        ...
        try:
            ...
            logging.info("[OK] Connected to SMTP server")
            ...
            logging.info("=" * 80)
            logging.info(f"[OK][OK][OK] EMAIL SENT SUCCESSFULLY to {len(all_recipients)} recipients!")
            logging.info("=" * 80)
        except Exception as e:
            logging.error("=" * 80)
            logging.error(f"[X][X][X] FAILED TO SEND EMAIL")
...
def main():
    ...
    logging.info(f"[Config] Execution Mode: {mode}")
    logging.info(f"[Date] Execution Frequency: Every {EXECUTION_FREQUENCY} day(s)")
    logging.info(f"[Email] Recipients: {len(EMAIL_CONFIG['recipients'])} recipient(s)")
    if TEST_MODE:
        logging.info(f"   [!]️  TEST MODE ACTIVE - Sending to: {', '.join(EMAIL_CONFIG['recipients'])}")
    else:
        logging.info(f"   [DONE] PRODUCTION MODE - Sending to {len(EMAIL_CONFIG['recipients'])} recipients")
    logging.info("=" * 80)
    logging.info("")
    ...
    try:
        ...
        logging.info(f"[OK] Data fetch completed. Records: {len(df_pending_tasks)}")
        ...
        logging.info(f"[OK] Summary table created with {len(df_summary)} task types")
        ...
        logging.info(f"[OK] HTML report saved: {HTML_FILE}")
        ...
        logging.info(f"[OK] Excel report saved: {EXCEL_FILE}")
