...
def load_json_data(filename):
    ...
    try:
        ...
        print(f"[ERROR] FATAL ERROR: JSON data file not found at {path}. Run convert_excel.py.")
        return []
...
def get_all_data():
    ...
    print(f"[INFO] Initial load: {len(sr_data)} total SRs, {len(defect_data)} total defects, {len(wa_data)} WAs")
    print(f"[INFO] Returning ALL SRs and Defects for initial display")
    
    return jsonify({
        "sr_data": sr_data,  # ALL SRs
        "defect_data": defect_data,  # ALL Defects
        "wa_data": wa_data,
...
def filter_data():
    ...
    try:
        conn = database.get_db_connection()
        if not conn:
            print("[ERROR] Database connection failed for WA data")
            wa_data = []
        else:
            cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        ...
        print(f"[ERROR] Database error in filter_data: {e}")
        wa_data = []
    ...
    if len(filtered_defect) > 0:
        ...
        print(f"[WARN] No defects found! Original defect_data had {len(defect_data)} records")
    print(f"{'='*60}\n")
