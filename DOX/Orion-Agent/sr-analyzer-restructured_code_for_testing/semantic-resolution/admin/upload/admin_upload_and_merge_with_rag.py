...
def upload_and_merge_with_rag(excel_path, progress_callback=None):
    ...
    try:
        ...
        for idx, sr_data in enumerate(sr_data_list, 1):
            ...
            if sr_result:
                # 🆕 Preserve Reported Date from original Excel data
                for r in sr_result:
            ...
            for idx, result in enumerate(results, 1):
                ...
                if sr_id in existing_sr_ids:
                    ...
                    success = hist_manager.update_sr_from_admin(
                        ...
                        reported_date=result.get('Reported Date')  # 🆕 Preserve date for age calculation
                    )
        ...
        # ==================================================
        # STEP 5.6: Final Cleanup (Input, Output, Email files)
        # ==================================================
        ...
        # Directories to clean up (all temp Excel files after data is in ChromaDB)
        cleanup_dirs = [
            ...
            ('Reports', Path(__file__).parent.parent.parent / 'output' / 'reports'),
        ]
