...
try:
    from scripts.utilities.priority_age_calculator import PriorityAgeCalculator
...
try:
    ...
    HISTORICAL_INDEXER_AVAILABLE = True
except ImportError:
    HISTORICAL_INDEXER_AVAILABLE = False
    logger.warning("HistoricalDataIndexer not found - historical_sr_index.pkl will not be used")
...
class AIEnhancedServiceRequestAnalyzer:
    ...
    def __init__(self):
        ...
        # Database paths
        self.java_db_path = Path("vector store/javaMapping.db")
        self.sr_db_path = Path("vector store/sr_tracking.db") 
        self.skills_db_path = Path("vector store/people_skills.db")
        self.historical_index_path = Path("vector store/historical_sr_index.pkl")
        self.java_classes_path = Path("vector store/comcast_java_classes.pkl")
        
        # 🆕 Try clean vectorstore first, fall back to regular one
        self.history_data_db_path = Path("vector store/clean_history_data.db")
        if not self.history_data_db_path.exists():
            self.history_data_db_path = Path("vector store/history_data.db")  # Fallback to old one
        
        # Directory paths
        self.input_dir = Path("DTU MOD/input")
        self.output_dir = Path("DTU MOD/llm output")
        ...
        self.preprocessor = None  # 🆕 For text preprocessing
        ...
        # Try to load NEW history_data.db first (priority)
        if self.history_data_db_path.exists() and SENTENCE_TRANSFORMERS_AVAILABLE:
            try:
                logger.info(f"🔄 Loading semantic vectorstore from {self.history_data_db_path}")
                with open(self.history_data_db_path, 'rb') as f:
                    self.historical_db = pickle.load(f)
                
                # Check if it's preprocessed
                is_preprocessed = self.historical_db.get('preprocessed', False)
                if is_preprocessed:
                    logger.info(f"   ✅ Using PREPROCESSED vectorstore (v{self.historical_db.get('preprocessing_version', '1.0')})")
                else:
                    logger.info(f"   ℹ️ Using raw (non-preprocessed) vectorstore")
                ...
                total_records = self.historical_db['total_records']
                logger.info(f"✅ NEW history_data.db loaded successfully!")
                logger.info(f"   📊 Records: {total_records:,}")
                logger.info(f"   🧠 Model: {self.historical_db['model_name']}")
                logger.info(f"   📏 Embedding dims: {self.historical_db['embeddings'].shape[1]}")
                logger.info(f"   🔍 Semantic search: ENABLED with Sentence Transformers")
            except Exception as e:
                logger.error(f"❌ Error loading history_data.db: {str(e)}")
                self.historical_db = None
                self.semantic_model = None
                
                # Fallback to old index
                if HISTORICAL_INDEXER_AVAILABLE:
                    try:
                        logger.info("⚠️ Falling back to old historical_sr_index.pkl...")
                        self.historical_indexer = HistoricalDataIndexer()
                        
                        phase1_nlp_path = Path("vector store/historical_sr_index_phase1_enhanced.pkl")
                        if phase1_nlp_path.exists():
                            self.historical_indexer.load_index(str(phase1_nlp_path))
                            self.index_loaded = True
                            self.phase1_enhanced = True
                            logger.info(f"✅ Fallback: Phase 1 NLP-enhanced index loaded")
                        elif self.historical_index_path.exists():
                            self.historical_indexer.load_index(str(self.historical_index_path))
                            self.index_loaded = True
                            logger.info(f"✅ Fallback: Historical index loaded")
                    except Exception as e2:
                        logger.error(f"❌ Fallback also failed: {str(e2)}")
                        self.historical_indexer = None
        elif not SENTENCE_TRANSFORMERS_AVAILABLE:
            logger.warning("⚠️ sentence-transformers not available - using old historical index")
            # Fallback to old index
            if HISTORICAL_INDEXER_AVAILABLE:
                try:
                    self.historical_indexer = HistoricalDataIndexer()
                    ...
                    if phase1_nlp_path.exists():
                        ...
                        self.index_loaded = True
                        self.phase1_enhanced = True
                        logger.info(f"✅ Using old Phase 1 NLP-enhanced index")
                except Exception as e:
                    logger.error(f"❌ Error loading fallback index: {str(e)}")
        else:
            logger.warning(f"⚠️ history_data.db not found at {self.history_data_db_path}")
        ...
        # Load Java classes index
        self.java_classes_data = None
        self.java_class_index = {}
        self.java_project_index = {}
        
        if self.java_classes_path.exists():
            try:
                logger.info(f"Loading comcast_java_classes.pkl from {self.java_classes_path}")
                with open(self.java_classes_path, 'rb') as f:
                    self.java_classes_data = pickle.load(f)
                self.java_class_index = self.java_classes_data.get('class_index', {})
                self.java_project_index = self.java_classes_data.get('project_index', {})
                total_classes = self.java_classes_data.get('total_classes', len(self.java_class_index))
                logger.info(f"✅ Java classes loaded: {total_classes} classes from {len(self.java_project_index)} projects")
            except Exception as e:
                logger.error(f"❌ Error loading Java classes: {str(e)}")
                self.java_classes_data = None
        ...
        logger.info("=" * 80)
        logger.info(f"Java DB: {'✅' if self.java_db_path.exists() else '❌'} {self.java_db_path}")
        ...
        logger.info(f"Historical Index: {'✅' if self.index_loaded else '❌'} {self.historical_index_path}")
        logger.info("=" * 80)
    ...
    def advanced_java_analysis(self, description: str, notes: str, category: str) -> Dict:
        ...
        conn = self.connect_database(self.java_db_path)
        if not conn:
            return java_context
    ...
    def semantic_search_history(self, description: str, notes: str = "", top_k: int = 5) -> List[Dict]:
        """
        NEW: Semantic search using history_data.db with Sentence Transformers
        Returns SR IDs with combined semantic workaround (resolution + wl_summary + workaround)
        Plus NEW SLA categorization fields
        ...
        """
        if not self.historical_db or not self.semantic_model:
            logger.debug("history_data.db or semantic model not available")
            return []
        ...
        try:
            ...
            db_is_preprocessed = self.historical_db.get('preprocessed', False)
            if db_is_preprocessed and self.preprocessor:
                original_query = query_text
                query_text = self.preprocessor.clean_for_semantic_search(query_text)
                logger.debug(f"   ✅ Query preprocessed (DB is preprocessed)")
                if len(query_text) < 10:
                    logger.debug("   ⚠️ Preprocessed query too short, using original")
                    query_text = original_query
            elif db_is_preprocessed and not self.preprocessor:
                logger.warning("   ⚠️ DB is preprocessed but preprocessor not available - matches may be suboptimal")
            ...
            embeddings = self.historical_db['embeddings']
            ...
            # Get top-k results (minimum 50% similarity)
            top_indices = np.argsort(similarities)[::-1][:top_k * 2]  # Get more initially to filter
            ...
            for idx in top_indices:
                ...
                metadata = self.historical_db['metadata'][idx]
                ...
                results.append({
                    ...
                    'user_corrected_workaround': metadata.get('user_corrected_workaround', ''),  # 🆕 NEW
                    # Individual fields for UI display
                    'summary': metadata.get('summary', ''),
                    'wl_summary': metadata.get('wl_summary', ''),
                    'workaround': metadata.get('workaround', ''),
                    # 🆕 NEW SLA Categorization fields
                    ...
                    'description': metadata.get('description', '')[:100],  # Truncated for context
                })
    ...
    def intelligent_team_assignment(self, application: str, complexity_score: float, priority: str, java_failure: bool) -> Dict:
        ...
        try:
            ...
            if top_candidates:
                ...
                # Add load balancing note
                reasoning_parts.append(f"Availability: {selected['availability']:.0%}")
    ...
    def analyze_single_sr(self, sr_data: Dict) -> Dict:
        ...
        try:
            ...
            result = {
                ...
                'Java Failure Detected': 'Yes' if java_context.get('java_failure_detected') else 'No',
                'Troubleshooting Steps': troubleshooting_steps,
                'Historical Cases Used': sum(1 for e in workaround_evidence if e.get('sr_id'))
                    or historical_context.get('similar_cases_count', 0),
                'Similar Cases': self.format_similar_cases(ai_results.get('similar_cases', [])[:3], historical_context),
                'Issue Type': issue_type,
                'Assigned To': assignment_context.get('assigned_to', 'NA'),
                'Success Rate': f"{ai_results.get('success_rate', historical_context.get('success_rate', 0)):.1%}",
                'AI Analysis': ai_analysis,
                'Recommended Action': recommended_action,
                'Application': assigned_group,
                'Confidence': f"{overall_confidence}%",
                'Functional Area': category,
                'Prediction Method': 'Semantic search + historical DB evidence',
                'Original Description': description,
                'Original Notes/Summary': notes,
                # NEW COLUMNS: Semantic workarounds from history_data.db
                'Semantic Workarounds': semantic_workaround_data['semantic_workarounds_list'] if semantic_workaround_data else 'No semantic matches found',
                'Previous AI Workarounds (Similar SRs)': semantic_workaround_data['ai_workarounds_list'] if semantic_workaround_data else 'No AI workarounds found',
                'User Corrections (Similar SRs)': semantic_workaround_data['user_corrections_list'] if semantic_workaround_data else 'No user corrections found',
                # 🆕 TWO SEPARATE CATEGORIZATION FIELDS - Prioritize original input, fallback to similar SRs
            ...
            return {
                ...
                'Expected Path': 'Manual Review Required',
                'Complexity': 'Unknown',
                'Interface': 'Unknown',
                'Risk': 'Unknown',
                'Suggested Workaround': 'NA',
                'Java Failure Detected': 'NA',
                'Troubleshooting Steps': 'NA',
                'Historical Cases Used': 0,
                'Similar Cases': 'NA',
                'Issue Type': 'Analysis Error',
                'Assigned To': 'NA',
                'Success Rate': 'NA',
                'AI Analysis': f'Analysis failed: {str(exc)}',
                'Recommended Action': 'Manual Review',
                'Application': sr_data.get('Assigned Group', 'Unknown'),
                'Confidence': '0%',
                'Functional Area': 'Unknown',
                'Prediction Method': 'Error - No Analysis Performed',
...
def main():
    ...
    try:
        ...
        if success:
            ...
            print("Check the 'DTU MOD/llm output' folder for results.")
