...
class PeopleSkillsDatabase:
    ...
    def _upgrade_schema_if_needed(self):
        """Upgrade database schema to add unique constraints and email column if needed"""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            ...
            # Check if email column exists in team_members table
            cursor.execute("PRAGMA table_info(team_members)")
            columns = [col[1] for col in cursor.fetchall()]
            
            if 'email' not in columns:
                print("[UPGRADE] Adding email column to team_members table...")
                cursor.execute("ALTER TABLE team_members ADD COLUMN email TEXT")
                
                # Set default email for existing users
                cursor.execute("UPDATE team_members SET email = 'test@amdocs.com' WHERE email IS NULL")
                
                conn.commit()
                print("[OK] Email column added and default values set for existing users")
    ...
    def get_team_configuration(self) -> Dict[str, Any]:
        ...
        for _, row in df.iterrows():
            ...
            app = row['application']
            # Safely parse specializations - handle both JSON and plain text
            if row['specializations']:
                try:
                    specializations = json.loads(row['specializations'])
                except json.JSONDecodeError:
                    # Handle plain text format
                    specializations = [s.strip() for s in str(row['specializations']).replace(',', ' ').split() if s.strip()]
            else:
                specializations = []
            ...
            for field, new_value in updates.items():
                if field == 'skill_level':
                    ...
                    if result is None:
                        ...
                        cursor.execute('''
                        INSERT INTO skills (member_id, application, skill_level, last_updated)
                        VALUES (?, ?, ?, CURRENT_TIMESTAMP)
                        ''', (member_id, app, new_value))
                        old_value = 0
                    else:
                        old_value = result[0]
                        # Update skill level
                    ...
                    # Full replacement of specializations
                    app = updates.get('application', 'SOM_MM')
                    
                    # Get current specializations for logging
                    cursor.execute('SELECT specializations FROM skills WHERE member_id = ? AND application = ?',
                                 (member_id, app))
                    result = cursor.fetchone()
                    old_specs = result[0] if result else '[]'
                    
                    # Parse new specializations (comma or space separated)
                    if isinstance(new_value, str):
                        spec_list = [s.strip() for s in new_value.replace(',', ' ').split() if s.strip()]
                    else:
                        spec_list = new_value if new_value else []
                    
                    new_specs_json = json.dumps(spec_list)
                    ...
                    if result:
                        ...
                        UPDATE skills SET specializations = ?, last_updated = CURRENT_TIMESTAMP
                        WHERE member_id = ? AND application = ?
                        ''', (new_specs_json, member_id, app))
                    else:
                        # Insert new skill entry if doesn't exist
                        cursor.execute('''
                        INSERT INTO skills (member_id, application, skill_level, specializations, max_load, last_updated)
                        VALUES (?, ?, 3.0, ?, 10, CURRENT_TIMESTAMP)
                        ''', (member_id, app, new_specs_json))
                    
                    # Log change
                    cursor.execute('''
                    INSERT INTO config_changes 
                    (change_type, member_id, field_name, old_value, new_value, changed_by)
                    VALUES ('SPECIALIZATION_UPDATE', ?, ?, ?, ?, ?)
                    ''', (member_id, f"{app}_specializations", old_specs, new_specs_json, changed_by))
    ...
    def add_member(self, name: str, email: str = '', status: str = 'active') -> bool:
        """
        Add a new team member to the database
        
        Args:
            name: Name of the team member
            email: Corporate email (must end with @amdocs.com)
        ...
        try:
            ...
            cursor.execute('SELECT id FROM team_members WHERE name = ?', (name,))
            existing = cursor.fetchone()
            
            if existing:
                self.logger.warning(f"Member '{name}' already exists")
                conn.close()
                return False
            
            # Validate email format
            if email and not email.lower().endswith('@amdocs.com'):
                self.logger.warning(f"Invalid email format: {email}. Must end with @amdocs.com")
                email = ''  # Set to empty if invalid
            
            # Insert new member with email
            cursor.execute('''
                INSERT INTO team_members (name, email, status)
                VALUES (?, ?, ?)
            ''', (name, email or 'test@amdocs.com', status))
            
            conn.commit()
            conn.close()
            
            self.logger.info(f"Added new team member: {name} with email: {email}")
            return True
            
        except Exception as e:
            self.logger.error(f"Error adding member: {str(e)}")
            return False
    
    def add_member_with_skills(self, name: str, application: str, 
                               skill_level: float, specializations: str, max_load: int,
                               email: str = '') -> bool:
        """
        Add a new team member with initial skill
        
        Args:
            name: Name of the team member
            application: Primary application (e.g., SOM_MM)
            skill_level: Skill level (1-5)
            specializations: Comma-separated specializations
            max_load: Maximum workload capacity
            email: Corporate email (must end with @amdocs.com)
        ...
        try:
            ...
            cursor.execute('''
                INSERT INTO team_members (name, email, status)
                VALUES (?, ?, 'active')
            ''', (name, email or 'test@amdocs.com'))
            
            member_id = cursor.lastrowid
            
            # Insert skill - convert specializations to JSON array
            spec_list = [s.strip() for s in specializations.replace(',', ' ').split() if s.strip()]
            spec_json = json.dumps(spec_list)
            ...
            self.logger.info(f"Added new team member: {name} with email: {email} and skill for {application}")
            return True
            
        except Exception as e:
            self.logger.error(f"Error adding member with skills: {str(e)}")
            return False
    ...
    def remove_member(self, name: str) -> bool:
        ...
        try:
            ...
            if not result:
                self.logger.warning(f"Member '{name}' not found")
                conn.close()
                return False
            
            member_id = result[0]
            
            # Delete related skills first (foreign key constraint)
            cursor.execute('DELETE FROM skills WHERE member_id = ?', (member_id,))
            ...
            self.logger.info(f"Permanently deleted member: {name}")
            return True
            
        except Exception as e:
            self.logger.error(f"Error removing member: {str(e)}")
            return False
    ...
    def get_all_people(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of dicts with member info including availability and email
        """
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            
            # Get all active team members including email
            cursor.execute('''
                SELECT DISTINCT tm.name, tm.status, tm.email
                FROM team_members tm
                WHERE tm.status = 'active'
            ''')
            ...
            for row in cursor.fetchall():
                ...
                # Get ALL applications this person has expertise in (including max_load)
                cursor.execute('''
                    SELECT skill_level, application, max_load
                    FROM skills
                    WHERE member_id = (SELECT id FROM team_members WHERE name = ?)
                ...
                if skills_results:
                    ...
                    # Get max_load from all skills (use highest)
                    max_load = max([s[2] if s[2] else 10 for s in skills_results])
                else:
                    skill_level = 'Intermediate'
                    applications = ['SOM_MM']
                    primary_app = 'SOM_MM'
                ...
                members.append({
                    'name': name,
                    'status': row[1],
                    'email': row[2] if len(row) > 2 else 'test@amdocs.com',  # Include email
                    'current_availability': availability,
                    'skill_level': skill_level,
                    'max_load': max_load,  # Include max_load from skills table
    ...
    def get_member_by_email(self, email: str) -> Optional[Dict[str, Any]]:
        """
        Get team member by their corporate email
        
        Args:
            email: Corporate email (e.g., jarwalk@amdocs.com)
        ...
        try:
            ...
            self.logger.error(f"Error getting member by email: {str(e)}")
            return None
    
    def update_member_email(self, member_name: str, new_email: str) -> bool:
        """
        Update a team member's email address
        
        Args:
            member_name: Name of the team member
            new_email: New email address (must end with @amdocs.com)
        ...
        try:
            ...
            if not new_email.lower().endswith('@amdocs.com'):
                self.logger.error(f"Invalid email format: {new_email}. Must end with @amdocs.com")
                return False
            ...
            cursor.execute('SELECT id, email FROM team_members WHERE name = ?', (member_name,))
            result = cursor.fetchone()
            ...
            cursor.execute('''
                UPDATE team_members 
                SET email = ?, updated_date = CURRENT_TIMESTAMP
                WHERE name = ?
            ...
            cursor.execute('''
                INSERT INTO config_changes 
                (change_type, member_id, field_name, old_value, new_value, changed_by)
                VALUES ('EMAIL_UPDATE', ?, 'email', ?, ?, 'ADMIN')
            ''', (result[0], old_email or '', new_email))
            
            conn.commit()
            conn.close()
            
            self.logger.info(f"Updated email for {member_name}: {old_email} -> {new_email}")
            return True
            
        except Exception as e:
            self.logger.error(f"Error updating member email: {str(e)}")
            return False
    
    def get_member_email(self, member_name: str) -> Optional[str]:
        """
        Get a team member's email address
        ...
        try:
            ...
            cursor.execute('SELECT email FROM team_members WHERE name = ? AND status = ?', 
                          (member_name, 'active'))
            result = cursor.fetchone()
            conn.close()
            ...
            self.logger.error(f"Error getting member email: {str(e)}")
            return None
