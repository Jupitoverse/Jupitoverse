...
# Text preprocessor for cleaning SR descriptions before embedding
try:
...
class HistoryDatabaseManager:
    ...
    def __init__(self, db_path: str = None, chromadb_path: str = None):
        ...
        """
        self.db_path = db_path  # Legacy pickle path
    ...
    def save_database(self) -> bool:
        """Save the history database (legacy - ChromaDB auto-persists)"""
        # ChromaDB auto-persists, no manual save needed
        if self.use_chromadb:
            logger.debug("ChromaDB auto-persists, no manual save needed")
            return True
        
        # Legacy pickle save
        if not self.db_path:
            logger.warning("No db_path set, skipping pickle save")
            return True
        ...
        **additional_fields
            ...
            try:
                ...
                if existing_results and existing_results['ids']:
                    ...
                    if ai_wa_str and ai_wa_str not in ['NA', 'N/A', '']:
                        existing_metadata['ai_generated_workaround'] = ai_wa_str[:10000]  # Increased limit
                    ...
                    metadata = {
                        ...
                        'workaround': str(additional_fields.get('workaround', ''))[:5000],
                        'ai_generated_workaround': ai_wa_str[:10000],  # Increased limit
        ...
        try:
            ...
            metadata = {
                ...
                'Reported Date': additional_fields.get('reported_date', None)  # For business day age calculation
            }
        ...
        if self.use_chromadb and self.chromadb_collection:
            try:
                ...
                if results and results['ids']:
                    ...
                    # Update metadata
                    old_metadata['ai_generated_workaround'] = str(new_ai_workaround)[:10000]  # Increased limit
                    old_metadata['ai_workaround_updated'] = datetime.now().isoformat()
                    old_metadata['ai_workaround_previous'] = str(old_workaround)[:1000]
        ...
        if self.use_chromadb and self.chromadb_collection:
            try:
                ...
                if ai_generated_workaround is not None:
                    existing_metadata['ai_generated_workaround'] = str(ai_generated_workaround)[:10000]  # Increased limit
    ...
    def _create_searchable_text(self, metadata: Dict[str, Any]) -> str:
        ...
        # Apply preprocessing if database was built with it
    ...
    def get_srs_by_assignee(self, assignee_name: str) -> List[Dict[str, Any]]:
        """
        Get all SRs assigned to a specific person.
        
        Args:
            assignee_name: Name of the assignee (case-insensitive partial match)
        
        Returns:
            List of SR records sorted by date (newest first)
        """
        if not self.use_chromadb or not self.chromadb_collection:
            logger.warning("ChromaDB not available for assignee lookup")
            return []
        
        try:
            # Get all records and filter by assignee
            all_results = self.chromadb_collection.get(
                include=["metadatas", "documents"]
            )
            ...
            for i, sr_id in enumerate(all_results['ids']):
                metadata = all_results['metadatas'][i] if all_results['metadatas'] else {}
                
                # Check if assigned to this person (case-insensitive partial match)
                assigned_to = metadata.get('assigned_to', '') or metadata.get('current_assignee', '') or ''
                if not assigned_to or assignee_lower not in assigned_to.lower():
                    continue
                
                # Build SR record
                sr_record = {
                    'sr_id': metadata.get('call_id', sr_id),
                    'description': (metadata.get('description', '') or '')[:150] + '...' if len(metadata.get('description', '') or '') > 150 else metadata.get('description', ''),
                    'priority': metadata.get('priority', metadata.get('Customer Priority', 'P3')),
                    'status': metadata.get('status', 'Open'),
                    'assigned_to': assigned_to,
                    'application': metadata.get('application', ''),
                    'reported_date': metadata.get('Reported Date', metadata.get('reported_date', '')),
                    'assigned_date': metadata.get('assigned_date', metadata.get('added_date', '')),
                    'has_ai_workaround': bool(metadata.get('ai_generated_workaround', '')),
                    'has_user_correction': bool(metadata.get('user_corrected_workaround', '')),
                }
                srs.append(sr_record)
            
            # Sort by date (newest first)
            srs.sort(
                key=lambda x: x.get('reported_date', '') or x.get('assigned_date', ''), 
                reverse=True
            )
            
            logger.info(f"Found {len(srs)} SRs assigned to {assignee_name}")
            return srs
            
        except Exception as e:
            logger.error(f"Error getting SRs for assignee {assignee_name}: {e}")
            return []
