...
def create_searchable_text_with_preprocessing(row, preprocessor):
    ...
    ✅ ONLY preprocess description field for MATCHING
    ...
    if 'description' in row.index:
        ...
        if value and value.lower() not in ['nan', 'none', '', 'na']:
            # Preprocess - remove customer/project noise for better semantic matching
    ...
    # 1. They're NOT used for matching (only description is)
...
def create_vectorstore_with_preprocessing(df, output_path='data/vectorstore/clean_history_data.db'):
    """
    ...
    db_data = {
        ...
        'metadata': metadata_list,  # Workarounds/resolutions stored unmodified
        ...
        'preprocessed': True,  # Flag: preprocessing applied to search text
        'preprocessing_version': '4.0',  # Updated - removed redundant 'summary' field
        'preprocessing_applied': {
            'fields_preprocessed': ['description'],  # Only description (summary was redundant)
...
def validate_vectorstore(db_path='data/vectorstore/clean_history_data.db'):
...
def main():
    ...
    output_path = 'data/vectorstore/clean_history_data.db'
