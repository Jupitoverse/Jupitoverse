...
WRITE_DB_CONFIG = {
    ...
    'user': 'ossdb01db',
    'password': 'Pr0d_ossdb01db',
    'connect_timeout': 120  # 2 minutes timeout (increased from 30s)
}
...
class EmailMgr:
    ...
    def send_mail(self):
        ...
        try:
            ...
            mode_suffix = " [REPORT-ONLY]" if REPORT_ONLY_MODE else ""
            MESSAGE['subject'] = f"Comcast OSS || OSO Service Activated Report{mode_suffix} - {Execution_Date}"
    ...
    def get_mail_content(self):
        ...
        <body>
        <h2>OSO Service Activated - Daily {'Report' if REPORT_ONLY_MODE else 'Sync Report'}</h2>
        
        <p>Hi Team,</p>
        
        <p>Please find below the execution summary for the <strong>OSO Service Activated Data {'Report' if REPORT_ONLY_MODE else 'Sync'}</strong> process that ran on <span class="highlight">{Execution_DateTime}</span>.</p>
        
        {'<div class="note" style="margin-bottom: 20px;"><strong>🔍 REPORT-ONLY MODE:</strong> No database writes were performed. This report shows what records would be inserted if running in normal mode.</div>' if REPORT_ONLY_MODE else ''}
        ...
        <tr>
            <td>{'NEW Records Found (would be inserted)' if REPORT_ONLY_MODE else 'New Records Inserted'}</td>
            <td class="highlight">{new_records}</td>
        </tr>
        <tr>
            <td>Existing Records {'(already in table)' if REPORT_ONLY_MODE else 'Skipped'}</td>
        ...
        <p>The attached Excel file contains {'multiple sheets with detailed analysis:' if REPORT_ONLY_MODE else f'<strong>ALL records</strong> currently in the tracking table ({table_total} records).'}</p>
        {'<ul><li><strong>All Records</strong>: Complete tracking table data (' + str(table_total) + ' records)</li><li><strong>NEW Records</strong>: Records that would be inserted (' + str(new_records) + ' records)</li><li><strong>EXISTING Records</strong>: Records already in table (' + str(existing_records) + ' records)</li></ul>' if REPORT_ONLY_MODE else '<ul><li>All data columns from the source query</li><li>Tracking columns: RCA, RCA_Category, Owned_By, WorkQueue, Task_Owner, Tracking_ID, Next_Action, Handling_Status</li><li>Metadata: created_at, updated_at timestamps</li></ul>'}
...
def check_new_vs_existing_records(df):
    """
    Check which records are new vs existing WITHOUT inserting.
    Used in REPORT_ONLY_MODE to identify what would be inserted.
    
    Args:
        df: pandas DataFrame with data to check
    
    Returns:
        tuple: (df_new_records, df_existing_records, new_count, existing_count)
    """
    read_conn = None
    cursor = None
    
    try:
        logger.info("Checking which records are new vs existing (REPORT-ONLY MODE)...")
        
        print("\n" + "=" * 80)
        print("REPORT-ONLY MODE: Checking records (no database writes)")
        print("=" * 80)
        
        read_conn = get_db_connection(READ_DB_CONFIG, "READ")
        if not read_conn:
            print("[ERROR] Could not connect to READ DB")
            return df, pd.DataFrame(), len(df), 0
        ...
        logger.info(f"Checking {len(df)} records against existing table...")
        ...
        for idx, row in df.iterrows():
            try:
                ...
                # Check if record exists using READ DB
                check_sql = f"""
                ...
                if (idx + 1) % 20 == 0:
                    print(f"    Progress: {idx + 1}/{len(df)} checked...")
        ...
        print(f"\n[OK] Check Complete:")
        print(f"    Total checked: {len(df)} records")
        print(f"    NEW records: {new_count} (would be inserted)")
        print(f"    EXISTING records: {existing_count} (already in table)")
        print("=" * 80)
        
        logger.info(f"[OK] NEW: {new_count}, EXISTING: {existing_count}")
        
        return df_new, df_existing, new_count, existing_count
        
    except Exception as e:
        logger.error(f"[ERROR] Error checking records: {e}")
        logger.debug(traceback.format_exc())
        print(f"[ERROR] Failed to check records: {e}")
        return df, pd.DataFrame(), len(df), 0
...
def save_to_excel(df, filename=None, df_fetched=None, df_new=None, df_existing=None):
    """
    Save DataFrame to Excel file for email attachment.
    Creates multiple sheets based on available data.
    
    Args:
        df: pandas DataFrame (all table data)
        filename: Output filename (auto-generated if None)
        df_fetched: Optional DataFrame with freshly fetched data
        df_new: Optional DataFrame with new records (REPORT_ONLY_MODE)
        df_existing: Optional DataFrame with existing records (REPORT_ONLY_MODE)
    ...
    try:
        if filename is None:
            ...
            mode_suffix = "_ReportOnly" if REPORT_ONLY_MODE else ""
            filename = f"OSO_Service_Activated{mode_suffix}_{timestamp}.xlsx"
        ...
        if REPORT_ONLY_MODE:
            print("  MODE: Report-Only (No Database Writes)")
        print("=" * 80)
        ...
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            # REPORT_ONLY_MODE sheets
            if REPORT_ONLY_MODE and df_new is not None and not df_new.empty:
                df_new.to_excel(writer, index=False, sheet_name='NEW Records')
                print(f"  Sheet 'NEW Records': {len(df_new)} rows (would be inserted)")
                logger.info(f"  Sheet 'NEW Records': {len(df_new)} rows")
            
            if REPORT_ONLY_MODE and df_existing is not None and not df_existing.empty:
                df_existing.to_excel(writer, index=False, sheet_name='EXISTING Records')
                print(f"  Sheet 'EXISTING Records': {len(df_existing)} rows (already in table)")
                logger.info(f"  Sheet 'EXISTING Records': {len(df_existing)} rows")
            
            # Sheet for fetched data from SQL query (if provided and not in REPORT_ONLY_MODE)
            if not REPORT_ONLY_MODE and df_fetched is not None and not df_fetched.empty:
        ...
        if os.path.exists(filename):
            ...
            print(f"    Total sheets: {2 + (1 if df_new is not None else 0) + (1 if df_existing is not None else 0) + (1 if not REPORT_ONLY_MODE and df_fetched is not None else 0)}")
            print("=" * 80)
...
if __name__ == "__main__":
    ...
    logger.info(f"Report-Only Mode: {'ENABLED' if REPORT_ONLY_MODE else 'DISABLED'}")
    ...
    # ==========================================================================
    # STEP 4: Insert Records or Generate Report (based on REPORT_ONLY_MODE)
    # ==========================================================================
    inserted = 0
    skipped = 0
    df_new_records = None
    df_existing_records = None
    ...
    if not dry_run and df_fetched_data is not None and not df_fetched_data.empty:
        if REPORT_ONLY_MODE:
            ...
            logger.info("STEP 4: REPORT-ONLY MODE - Checking new vs existing records (no writes)")
            
            print(f"Will check {len(df_fetched_data)} records against existing table...")
            print("[INFO] REPORT_ONLY_MODE is enabled - no database writes will be performed")
            
            # Check which records are new vs existing (no inserts)
            df_new_records, df_existing_records, inserted, skipped = check_new_vs_existing_records(df_fetched_data)
            
            logger.info(f"NEW records found: {inserted}, EXISTING records: {skipped}")
        else:
    ...
    if df_all_data is not None and not df_all_data.empty:
        # Pass all data + new/existing split (for REPORT_ONLY_MODE) or fetched data (for normal mode)
        excel_file = save_to_excel(
            df=df_all_data, 
            filename=None, 
            df_fetched=df_fetched_data if 'df_fetched_data' in locals() and not REPORT_ONLY_MODE else None,
            df_new=df_new_records if 'df_new_records' in locals() and REPORT_ONLY_MODE else None,
            df_existing=df_existing_records if 'df_existing_records' in locals() and REPORT_ONLY_MODE else None
        )
    else:
    ...
    elif REPORT_ONLY_MODE:
        print("[OK] REPORT-ONLY MODE COMPLETED!")
    ...
    print("-" * 80)
    if df_fetched_data is not None:
        print(f"Records fetched from READ DB: {len(df_fetched_data)}")
    if not dry_run:
        if REPORT_ONLY_MODE:
            print(f"NEW records found: {inserted} (not inserted - report-only mode)")
            print(f"EXISTING records: {skipped}")
        else:
