...
def get_db_connection(db_config, db_type="READ"):
    ...
    try:
        ...
        logger.info(f"[OK] {db_type} database connection established")
        return connection
...
def insert_new_records(df):
    ...
    Checks for existing records based on service_id + site_id + version.
    ...
    try:
        logger.info("Inserting new records into WRITE DB...")
        ...
        for idx, row in df.iterrows():
            try:
                ...
                if not service_id or not site_id or not version:
...
def fetch_all_table_data():
    ...
    try:
        logger.info("Fetching ALL records from tracking table...")
        ...
        results = cursor.fetchall()
        logger.info(f"Fetched {len(results)} total records from table")
...
def save_to_excel(df, filename=None):
    ...
    try:
        ...
        df.to_excel(filename, index=False, engine='openpyxl', sheet_name='Service Activated')
...
if __name__ == "__main__":
    ...
    if not dry_run:
        ...
        logger.info("STEP 2: Creating/Verifying table")
        
        table_created = create_table_if_not_exists()
        
        if not table_created:
            logger.error("Failed to create/verify table")
            print("[ERROR] Failed to create/verify table. Cannot proceed.")
            sys.exit(1)
    else:
        print("\nStep 2: [SKIPPED in dry-run mode]")
    
    # ==========================================================================
    # STEP 3: Fetch Service Activation Data
    # ==========================================================================
    print("\nStep 3: Fetching service activation data...")
    logger.info("STEP 3: Fetching data from READ DB")
    
    df_new_data = fetch_service_activation_data()
    ...
    if df_new_data is None or df_new_data.empty:
        ...
        print(f"\n[OK] Retrieved {len(df_new_data)} records from READ DB")
        
        # Display sample
        print("\nSample Data (first 3 rows):")
        print("-" * 80)
        print(df_new_data.head(3).to_string())
        print("-" * 80)
    ...
    if not dry_run and df_new_data is not None and not df_new_data.empty:
        print("\nStep 4: Inserting new records...")
        logger.info("STEP 4: Inserting records into WRITE DB")
        
        inserted, skipped = insert_new_records(df_new_data)
        
        logger.info(f"Inserted: {inserted}, Skipped: {skipped}")
    else:
        print("\nStep 4: [SKIPPED - dry-run mode or no data]")
        if df_new_data is not None:
            print(f"    Would process {len(df_new_data)} records")
    
    # ==========================================================================
    # STEP 5: Fetch All Table Data for Email
    # ==========================================================================
    print("\nStep 5: Fetching all table data for email...")
    logger.info("STEP 5: Fetching all table data")
    ...
    if df_all_data is None or df_all_data.empty:
        ...
        df_all_data = df_new_data  # Fallback to new data
    else:
    ...
    if df_all_data is not None and not df_all_data.empty:
        excel_file = save_to_excel(df_all_data)
    else:
    ...
    if not dry_run and SEND_EMAIL and excel_file:
        ...
        summary_stats = {
            'total_records': len(df_new_data) if df_new_data is not None else 0,
        ...
        # Send email
        email_mgr = EmailMgr(excel_file, summary_stats)
