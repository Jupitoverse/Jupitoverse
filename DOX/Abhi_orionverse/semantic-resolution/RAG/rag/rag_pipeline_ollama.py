"""
Enhanced SR Analysis RAG Pipeline with Ollama
Intelligent Service Request Analysis using Multiple Vectorstores
...
4. Input Excel - Resolution Category, Status Reason
"""
...
class VectorstoreHandler:
    ...
    def __init__(self, java_db_path: Path, comcast_code_db_path: Path, history_db_path: Path):
        ...
        # Load semantic model for vectorstore searches
    ...
    def get_java_metadata_context(self) -> str:
        ...
        try:
            ...
            # Query java_classes table for structured data
            query = """
            SELECT 
                class_name,
                package,
    ...
    def search_java_code_semantically(self, query_text: str, top_k: int = 5) -> List[Dict]:
        ...
        try:
            ...
            # Calculate similarity with all code chunks
    ...
    def search_historical_srs(self, query_text: str, top_k: int = 5) -> List[Dict]:
        ...
        try:
            ...
            # Calculate similarity with all historical SRs
            similarities = cosine_similarity(
                [query_embedding],
...
class SRAnalysisPipeline:
    ...
    def read_excel_input(self, file_path: Path) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            print(f"[INFO] Columns: {', '.join(df.columns[:10])}{'...' if len(df.columns) > 10 else ''}")
    ...
    def analyze_single_sr(self, sr_data: Dict, java_metadata_context: str) -> Dict:
        """Analyze a single service request using LLM with optimized context"""
        
        sr_id = sr_data.get('SR ID', sr_data.get('Call ID', 'Unknown'))
        priority = sr_data.get('Customer Priority', sr_data.get('Priority', 'Medium'))
...
"""
    ...
    def process_all_srs(self, df: pd.DataFrame):
        """Process all SRs with optimized context loading"""
        print(f"\n{'='*80}")
        print(f"[OPTIMIZE] Loading Java metadata context once for all SRs...")
        print(f"{'='*80}")
        
        # Load Java metadata once
        java_metadata_context = self.vectorstore.get_java_metadata_context()
        
        print(f"\n{'='*80}")
        print(f"[START] Starting SR Analysis Pipeline")
        print(f"{'='*80}")
        print(f"[INFO] Total SRs to process: {len(df)}")
        
        for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing SRs"):
            try:
                sr_data = row.to_dict()
                result = self.analyze_single_sr(sr_data, java_metadata_context)
                ...
                self.results.append({
                    ...
                    'Resolution Category': row.get('Resolution Category', 'Unknown'),
                    'Status Reason': row.get('Status Reason', 'Unknown'),
                    'Java Failure Detected': 'Error',
                    'Java Failure Path': 'N/A',
                    'Match Similarity': 'N/A',
                    'Semantic Workaround': str(row.get('Semantic Workaround', 'Not available')),
    ...
    def save_results(self, input_filename: str):
        ...
        try:
            ...
            output_filename = f"{base_name}_analysis_{date_str}_{time_str}.xlsx"
            ...
            print(f"[OUTPUT] Date: {now.strftime('%Y-%m-%d')} Time: {now.strftime('%H:%M:%S')}")
            print(f"[STATS] Total SRs analyzed: {len(self.results)}")
            print(f"[STATS] Java failures detected: {sum(1 for r in self.results if r['Java Failure Detected'] == 'Yes')}")
            print(f"{'='*80}")
    ...
    def run(self):
        ...
        print("\n" + "="*80)
        print("SR Analysis RAG Pipeline with Ollama - Enhanced Version")
        print("="*80)
...
def main():
    """Entry point"""
    try:
        pipeline = SRAnalysisPipeline(ollama_model="qwen2.5-coder:14b-instruct-q8_0")
