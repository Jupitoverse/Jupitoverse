...
function pull_data_activities {
  ...
  "SELECT ord.entity_name, oai.spec_ver_id,
          SUM(CASE WHEN oai.actual_start_date >= now() - interval '1 day'
                   AND oai.actual_start_date < now() THEN 1 ELSE 0 END) AS \"last 24 hours\"
   ...
   GROUP BY ord.entity_name, oai.spec_ver_id;" -o activities.html
  (( $? != SUCCESS )) && return $FAILURE
  return $SUCCESS
}
